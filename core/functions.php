...
if ( ! function_exists( 'starter_post_navigation' ) ) :
	 ...
	 * Lf Post Navigation
	 */
	function starter_post_navigation($custom_query = false) {
		?>
		<div class="nav-links">
		<?php
		global $wp_query;
...
if( function_exists('acf_add_options_page') ) {
    acf_add_options_page(
        array(
            'page_title'    => __('Site options', 'starter'),
            'menu_title'    => __('Site options', 'starter'),
...
function starter_customizer_changes( $wp_customize ) {
...
add_action( "customize_register", "starter_customizer_changes", 50 );
...
function starter_allowed_block_types() {
    global $pagenow;
    ...
    if( $pagenow != 'widgets.php' ):
        ...
        if( isset($block_post_type_slugs[$post_type]) && $block_post_type_slugs[$post_type] ):
            return $block_post_type_slugs[$post_type];
        else:
            global $block_slugs;
...
add_filter( 'allowed_block_types_all', 'starter_allowed_block_types' );
...
function embed_critical_css() {
    ...
    // if gutenberg
        ...
        // if page with ID
        elseif( ($ID > 1 && !is_search() && !is_archive() && !is_category() && !is_tag() && !is_tax() && !is_single() && !is_home()) || ($ID > 1 && (is_single() || is_home()) && has_blocks(get_the_content()) ) ):
            ...
            if( strpos($template_path, 'templates/') !== false ):
                ...
                if( has_blocks(get_the_content()) && (is_single() || is_home()) ):
                    global $template; 
